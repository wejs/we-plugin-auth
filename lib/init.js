module.exports = function initPlugin(sails, cb) {
  ...
  var weOauth2 = require('./oauth/index.js');
  ...
  sails.on('router:before',function onOauthPluginRouterBefore() {
    ...
    sails.router.bind('/*', function setDefaultLocalsVars(req, res, next) {
      ...
      res.locals.returnUrl = req.param('returnUrl');
  ...
  sails.on('we:passport:bind:after',function afterBindPassport() {
    ...
    return sails.router.bind('/*', function checkIfNeedToRedirect(req, res, next) {
      ...
      var sessionReturnUrl = req.session.returnUrl;
      ...
      if ( req.isAuthenticated() ) {
        if ( sessionService ) {
          ...
          if ( sessionReturnUrl ) {
            res.locals.returnUrl = sessionReturnUrl;
          ...
          if( service ) {
            ...
            var landingPage = sails.config.auth.landingPage;
            if ( !landingPage ) return sails.log.warn('No landingPage was set in auth.js config object');
            return sails.config.auth.callLandingPage(landingPage, req, res, next);
          }
        }
