...
util.parseToken = function parseToken(req) {
  ...
  if (req.cookies) {
    // get from cookie
    if (req.cookies && req.cookies[req._sails.config.auth.cookieName]) {
      return req.cookies[req._sails.config.auth.cookieName];
    } else if ( req.socket && req.socket.handshake ) {
      // is socket.io request
      if(req.socket.handshake.cookie[req._sails.config.auth.cookieName])
        return req.socket.handshake.cookie[req._sails.config.auth.cookieName];
    }
  }
...
util.expireToken = function expireOneToken(token, cb) {
  ...
  .exec(function queryResult(err,results) {
    if (err) {
      sails.log.error(err);
      return cb(err);
    }
